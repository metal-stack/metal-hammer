...
// func main() {
// 	// You need a real directory hierarchy to chroot into.
// 	// For a quick test: sudo mkdir -p /tmp/mychroot/bin /tmp/mychroot/etc
// 	//                   echo "hello from chroot" > /tmp/mychroot/etc/greeting
// 	chrootDir := "/tmp/mychroot"

// 	fmt.Println("=== Before chroot ===")
// 	printCwd()

// 	err := RunInChroot(chrootDir, func() error {
// 		fmt.Println("\n=== Inside chroot ===")
// 		printCwd()

// 		// Example: read a file that only exists inside the chroot
// 		data, err := os.ReadFile("/etc/greeting")
// 		if err != nil {
// 			fmt.Printf("  (could not read /etc/greeting: %v)\n", err)
// 		} else {
// 			fmt.Printf("  /etc/greeting says: %s\n", data)
// 		}

// 		// Any os.* or syscall.* call here runs inside the chroot
// 		return nil
// 	})

// 	if err != nil {
// 		fmt.Fprintf(os.Stderr, "chroot run failed: %v\n", err)
// 		os.Exit(1)
// 	}

// 	fmt.Println("\n=== After chroot (back to real root) ===")
// 	printCwd()

// 	// Prove we're really back by reading a file that only exists on the real FS
// 	if _, err := os.Stat("/proc/1"); err == nil {
// 		fmt.Println("  /proc/1 is accessible — we are back on the real root ✓")
// 	}
// }
